	...
	protected function connect($server) {
		...
		if (!$stream) {
			...
			if ($err)
				...
				show_error("APN Failed to connect: $err $errstrS");
...
*******************************/
	...
	function __construct() {
		...
		$this->_ci->config->load('apn_user',true);


		if(!file_exists($this->_ci->config->item('PermissionFileUser','apn_user')))
		{
			show_error("APN Failed to connect: APN Permission file not found");
		}

		$this->pushServer = $this->_ci->config->item('Sandbox','apn_user') ? $this->_ci->config->item('PushGatewaySandbox','apn_user') : $this->_ci->config->item('PushGateway','apn_user');
		$this->feedbackServer = $this->_ci->config->item('Sandbox','apn_user') ? $this->_ci->config->item('FeedbackGatewaySandbox','apn_user') : $this->_ci->config->item('FeedbackGateway','apn_user');

		$this->keyCertFilePath = $this->_ci->config->item('PermissionFileUser','apn_user');
		$this->passphrase = $this->_ci->config->item('PassPhrase','apn_user');
		$this->timeout = $this->_ci->config->item('Timeout','apn_user') ? $this->_ci->config->item('Timeout','apn_user') : 60;
		$this->expiry = $this->_ci->config->item('Expiry','apn_user') ? $this->_ci->config->item('Expiry','apn_user') : 86400;
	}
