...
class Driver extends CI_Controller {
	...
	public $tabel = 'tm_driver';
	public $field = 'driver_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			...
			// set sub active
			$this->session->set_userdata('sub_active', 'driver');
		}
		else {
    ...
    function index() {
		...
		$this->load->view('driver/v_list', $data);
	...
	function add_data() {
		...
		$this->load->view('../v_header');
		$this->load->view('../v_top');
		$this->load->view('driver/v_add', $data);
		$this->load->view('../v_bottom');
		$this->load->view('../v_footer');
    }
	...
	function insert_data() {
		...
		// cek exist
		if($this->m_global->check_existing($this->tabel, 'driver_code', $code, 'hospital_id', $hospital) == TRUE) {
			$this->session->set_flashdata('message', 'Code already exist.');
			$this->session->set_flashdata('status', get_notify_status(9));
			
			get_redirecting('master/driver/add-data');
		}
		
		// config upload
		$file 						= "";	
		$upload_path				= "./assets/uploads/driver";
		...
		$data = array(
			...
			'driver_image'		=> $file,
			'driver_code'		=> $code,
			'driver_name'		=> handling_characters($name),
			'driver_phone'		=> $phone,
			'driver_address'	=> (($address == "")?NULL:nl2br($address)),
	...
	function update_data() {
		...
		// cek old
		$old_code = $this->m_driver->get_code_by_id($id);
		...
		if($old_code != $code) {
			if($this->m_global->check_existing($this->tabel, 'driver_code', $code, 'hospital_id', $hospital) == TRUE) {
				...
				get_redirecting('master/driver/edit-data/'. simple_encrypt($id));
			}
		}
		
		// cek old
		$old_hospital = $this->m_driver->get_hospital_by_id($id);
		...
		$img = $this->m_driver->get_file_by_id($id);
		if ( ! $this->upload->do_upload()) {
			$file = $img;	
			$error = array('error' => $this->upload->display_errors());
			$this->session->set_flashdata('message', $error['error']);
