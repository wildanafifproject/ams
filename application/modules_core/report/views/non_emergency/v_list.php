				...
				<div class="col-lg-12">
					<div class="widget-container fluid-height clearfix">
						<div class="widget-content padded">
							<form class="form-horizontal" method="post" action="<?php echo base_url(); ?>report/non-emergency">
					...
					<table class="table table-bordered table-striped" id="dataTableNoSort">
						<thead>
							<tr>
								...
								<th width="15%" class="hidden-xs">Phone No.</th>
								<th width="15%">Ambulance</th>
								<th width="20%" class="hidden-xs">Booking Datetime</th>
						...
						<tbody>
							...
							<tr>
								<td align="center" class="hidden-xs"><?php echo convert_to_dmy($row->nonemergency_date); ?></td>
								<td align="center" class="hidden-xs"><?php echo convert_to_his($row->nonemergency_time); ?></td>
								<td><?php echo strip_tags($row->nonemergency_infoname); ?></td>
								<td align="center" class="hidden-xs"><?php echo $row->nonemergency_infophone; ?></td>
								<td align="center"><?php echo strip_tags($this->m_ambulance->get_plat_by_id($row->ambulance_id)); ?></td>
								<td align="center" class="hidden-xs"><?php echo (($row->nonemergency_infodate == "")?"-":convert_to_dmy($row->nonemergency_infodate) .' '. convert_to_his($row->nonemergency_infotime)); ?></td>
								<td align="center">
								...
								<td class="actions" align="center">
									<div class="action-buttons">
										<a class="table-actions" href="javascript:void(0);" onClick="link_detail('report/non-emergency', '<?php echo simple_encrypt($row->nonemergency_id); ?>');"><i class="icon-list-alt"></i></a>
									</div>
								</td>
							</tr>
							<?php endforeach; ?>
