...
class Specialist extends CI_Controller {
	...
	public $tabel = 'tm_specialist';
	public $field = 'specialist_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			if($this->session->userdata('user_authority') == 0) { 
				...
				$this->load->model('m_specialist');
				
				// set nav active
				$this->session->set_userdata('nav_active', 'setting');
				
				// set sub active
				$this->session->set_userdata('sub_active', 'specialist');
			}
			else {
    ...
    function index() {
		...
		$this->load->view('specialist/v_list', $data);
	...
	function add_data() {
		...
		$this->load->view('../v_header');
		$this->load->view('../v_top');
		$this->load->view('specialist/v_add');
		$this->load->view('../v_bottom');
		$this->load->view('../v_footer');
    }
	...
	function insert_data() {
		...
		if($valid != 1) {
			get_redirecting('setting/specialist');
		}
		...
		$data = array(
			...
			'specialist_name'	=> handling_characters($name),
	...
	function edit_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('specialist/v_edit', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
