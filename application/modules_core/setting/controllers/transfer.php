...
class Transfer extends CI_Controller {
	...
	public $tabel = 'tm_transfer';
	public $field = 'transfer_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			if($this->session->userdata('user_authority') == 0) { 
				...
				// set sub active
				$this->session->set_userdata('sub_active', 'transfer');
			}
			else {
    ...
    function index() {
		...
		$this->load->view('transfer/v_list', $data);
	...
	function insert_data() {
		...
		if($valid != 1) {
			get_redirecting('setting/transfer');
		}
		...
		$data = array(
			...
			'transfer_name'		=> handling_characters($name),
	...
	function edit_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('transfer/v_edit', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
