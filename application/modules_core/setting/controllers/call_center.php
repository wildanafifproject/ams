...
class Call_center extends CI_Controller {
	...
	public $tabel = 'tm_callcenter';
	public $field = 'callcenter_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			if($this->session->userdata('user_authority') == 0) { 
				...
				// set sub active
				$this->session->set_userdata('sub_active', 'center');
			}
			else {
    ...
    function index() {
		...
		$this->load->view('callcenter/v_list', $data);
	...
	function add_data() {
		...
		$this->load->view('../v_header');
		$this->load->view('../v_top');
		$this->load->view('callcenter/v_add');
		$this->load->view('../v_bottom');
		$this->load->view('../v_footer');
    }
	...
	function insert_data() {
		...
		if($valid != 1) {
			get_redirecting('setting/call-center');
		}
		...
		// set array
		$data = array(
			'callcenter_status'	=> $status,
			'callcenter_name'	=> handling_characters($name),
	...
	function edit_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('callcenter/v_edit', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
