...
class Location extends CI_Controller {
	...
	public $tabel = 'tm_location';
	public $field = 'location_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			if($this->session->userdata('user_authority') == 0) { 
				...
				// set sub active
				$this->session->set_userdata('sub_active', 'location');
			}
			else {
    ...
    function index() {
		...
		$this->load->view('location/v_list', $data);
	...
	function insert_data() {
		...
		if($valid != 1) {
			get_redirecting('setting/location');
		}
		...
		$data = array(
			...
			'location_name'		=> handling_characters($name),
			'area_id'			=> $area,
	...
	function edit_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('location/v_edit', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
