...
class sub_category extends CI_Controller {
	...
	public $tabel = 'tm_subcategory';
	public $field = 'subcategory_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			if($this->session->userdata('user_authority') == 0) { 
				...
				// set sub active
				$this->session->set_userdata('sub_active', 'subcategory');
			}
			else {
    ...
    function index() {
		...
		$this->load->view('subcategory/v_list', $data);
	...
	function add_data() {
		...
		$this->load->view('../v_header');
		$this->load->view('../v_top');
		$this->load->view('subcategory/v_add');
		$this->load->view('../v_bottom');
		$this->load->view('../v_footer');
    }
	...
	function insert_data() {
		...
		if($valid != 1) {
			get_redirecting('setting/sub-category');
		}
		...
		$data = array(
			...
			'subcategory_name'		=> handling_characters($name),
	...
	function edit_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('subcategory/v_edit', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
